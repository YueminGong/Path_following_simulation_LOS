...
for j=1:2400
    ...
    whole_angle=atan2(err_y,err_x); %% U(0) = U SITA
    ...
    rang=abs(trans'*[position(2)-trace(i+1,2);position(1)-trace(i+1,1)]);  %% equation(3)
    predict_path_angle=whole_angle-atan(appendage_coordinate(2)/delta);    %% equation(18)
...
Minv = inv(M);
acceleration=Minv*(-Nv*condition+force);
end